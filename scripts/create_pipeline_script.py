...
"""
        ...
        announce('Various pipeline parameters')
...
peak data available on the web for integration with UCSC genome browser."""
        ...
        ucsc_args = ''
        stage_dir = '%s/%s'%(all_tool_settings['web_stage'].get('stage_dir'),getpass.getuser())
        stage_url = '%s/%s'%(all_tool_settings['web_stage'].get('stage_url'),getpass.getuser())
        if ucsc_integrate :
            ucsc_args = ['--ucsc']
        ...
        ########################################################
        announce('MEME motif discovery settings')
        meme_text = """\
Below are the motif databases available on this system for meme-chip.  If you do not see the database that you need
may add your own to %(local_tool)s.  See %(glob_tool)s for details."""
        print textwrap.fill(meme_text%{'local_tool':LOCAL_TOOL_SETTINGS_FN,'glob_tool':GLOBAL_TOOL_SETTINGS_FN},break_long_words=False)
        print
        meme_args = []
        meme_settings = all_tool_settings['meme-chip']
        valid_meme_motifdb = meme_settings.get('db_sets','').split()
        wb('Available settings\n')
        for db in valid_meme_motifdb:
            wb(db.ljust(16))
            print ':', meme_settings.get(db+'.description','No description')
        meme_motifdb = ''
        while meme_motifdb not in valid_meme_motifdb :
            meme_motifdb = input('\nChoose MEME motif database, one of ('+','.join(valid_meme_motifdb)+')')
            print
        meme_args.append('-db=%s'%meme_settings[meme_motifdb+'.loc'])
        pipeline_args['--meme-args'] = ' '.join(meme_args)
        json_dict['meme motif db'] = meme_motifdb

        ##########################################################
        # Motif discvoery with THEME
        ##########################################################
        announce('THEME motif discovery settings')
        theme_args = ['fg_fn','bg_fn']
        theme_settings = all_tool_settings['theme']
        theme_hyp_text = """\
Below are the motif hypotheses files available on this system for THEME.  If you do not see the database that you need you
may add your own to %(local_tool)s.  See %(glob_tool)s for details."""
        print textwrap.fill(theme_hyp_text%{'local_tool':LOCAL_TOOL_SETTINGS_FN,'glob_tool':GLOBAL_TOOL_SETTINGS_FN},break_long_words=False)
        print
        valid_theme_hyp = theme_settings.get('hyp_sets','').split()
        wb('Available settings\n')
        for hyp in valid_theme_hyp:
            wb(hyp.ljust(8))
            print ':', theme_settings.get(hyp+'.description','No description')
        theme_hyp = ''
        while theme_hyp not in valid_theme_hyp :
            theme_hyp = input('\nChoose THEME motif hypotheses file, one of ('+','.join(valid_theme_hyp)+')')
            print
        theme_args.append(theme_settings[theme_hyp+'.loc'])
        json_dict['theme motif hypotheses'] = theme_hyp

        theme_markov_text = """\
Below are the Markov background files available on this system for THEME.  If you do not see the database that you need you
may add your own to %(local_tool)s.  See %(glob_tool)s for details."""
        print textwrap.fill(theme_markov_text%{'local_tool':LOCAL_TOOL_SETTINGS_FN,'glob_tool':GLOBAL_TOOL_SETTINGS_FN},break_long_words=False)
        print

        valid_theme_markov = theme_settings.get('markov_sets','').split()
        wb('Available settings\n')
        for markov in valid_theme_markov:
            wb(markov.ljust(8))
            print ':', theme_settings.get(markov+'.description','No description')
        theme_markov = ''
        while theme_markov not in valid_theme_markov :
            theme_markov = input('\nChoose THEME Markov background file, one of ('+','.join(valid_theme_markov)+')')
            print
        theme_args.append(theme_settings[theme_markov+'.loc'])
        json_dict['theme markov'] = theme_markov

        pipeline_args['--theme-args'] = ' '.join(theme_args)

        ##########################################################
        # info site
        ##########################################################
        infosite_args = []
        pipeline_args['--build-infosite-args'] = ' '.join(infosite_args)
